...
(function($) {
  ...
  if (typeof URL_BASE !== 'undefined') {
    config.baseUrl = URL_BASE + 'index.php';
  }
  else {
    config.baseUrl = '/';
  }
  GdBase.config = config;
  ...
  var utils = {
    ...
    handleError: function(xhr) {
      ...
      if (exception.type == utils.exceptions.sessionEnded) {
        if (parent) {
          parent.window.location = GdBase.config.baseUrl;
        }
        else {
          window.location = GdBase.config.baseUrl;
        }
      }
    ...
    post: function(route, data, success, complete, error) {
      ...
      jqXhr = $.ajax({
        ...
        error: function(xhr) {console.log('toto');return;
